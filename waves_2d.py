...
water_visc_dynamic = 1 #mPa.s = newton-second / m^2 = kg / m / s -- at ~20 degrees celsius. This is dynamic viscosity
water_visc_kinematic = water_visc_dynamic / rho_water #m^2 / s
water_visc_dynamic_nd = water_visc_dynamic * L * time / m_nd
water_visc_nd = water_visc_kinematic * time / L**2
air_visc_nd = water_visc_nd/16 # Define air viscosity here, starting from high to low   ###
...
#Two-fluid terms
rho = rho_water_nd* alpha + rho_air_nd*(1-alpha)  # true density
true_visc = water_visc_nd * alpha + air_visc_nd*(1-alpha)  # true viscosity
alpha_diffusion = water_visc_nd/16 # define alpha diffusion                      ###
...
problem.add_equation("dt(u) - water_visc_nd*div(grad_u) + grad(p)  + lift(tau_u2) = (1-alpha)*(air_visc_nd-water_visc_nd)*div(grad_u) - F_grav - (u@grad(u)) + F_st")
#problem.add_equation("dt(u) - water_visc_nd*div(grad_u) = (1-alpha)*(air_visc_nd-water_visc_nd)*div(grad_u)")
#problem.add_equation("dt(u) - true_visc*div(grad_u) + grad(p)  + lift(tau_u2) = - F_grav - (u@grad(u)) + F_st")
#problem.add_equation("dt(u) - water_visc_nd*div(grad_u) + grad(p)  + lift(tau_u2) = - F_grav - (u@grad(u)) + F_st")
...
alpha['g'] = 1 - 1/(1+np.exp(-(z-(0.5+0.2*np.sin(2*np.pi*x/Lx)))/0.025)) # use this
...
# Analysis
# snapshots = solver.evaluator.add_file_handler('snapshots', sim_dt=0.01, iter=20, max_writes=50)
snapshots = solver.evaluator.add_file_handler('snapshots', sim_dt=0.01, max_writes=100)
...
flow.add_property(np.sqrt(u@u)/water_visc_nd, name='Re')
